 ...
 * colored by health status. Uses CategoricalLegend for health categories.
 * Tooltip shows cluster detail with drill-down link.
 */
...
export const ClusterMap = ({
  ...
  /* Centre the map on Finland (average of all clusters, fallback) */
  const center = useMemo(() => {
    if (regions.length === 0) return { latitude: 63.0, longitude: 25.0, zoom: 4.5 };
  ...
  return (
    ...
    >
      <MapView height={height} initialViewState={center}>
        <BubbleLayer
          data={bubbleData}
          color={colorAccessor}
          scale="log"
        ...
        {!mini && <CategoricalLegend colorPalette={legendPalette} position="bottom" />}
      </MapView>
    </div>
  );
};
