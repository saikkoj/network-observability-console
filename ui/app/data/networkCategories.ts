...
export const NETWORK_CATEGORIES: NetworkCategory[] = [
  /* ── 1. Global Overview ────────────────────────── */
  {
    ...
    chartQuery: [
      `fetch dt.davis.problems`,
      `| filter in(affected_entity_types, "dt.entity.network:device")`,
      `| summarize count = countDistinctExact(dt.davis.event_ids), by:{bin(timestamp, 10m)}`,
      `| sort count desc`,
    ].join('\n'),
...
export const NETWORK_QUERIES = {
  ...
  deviceInventory: [
    ...
    `  fetch dt.davis.problems`,
    `  | filter in(affected_entity_types, "dt.entity.network:device")`,
    `  | summarize problems=countDistinctExact(dt.davis.event_ids), by:{affected_entity_ids}`,
  ...
  interfaceHealth: [
    ...
    `  | fieldsAdd errorsTotal = arraySum(errIn) + arraySum(errOut)`,
    `  | fieldsAdd discardsTotal = arraySum(discIn) + arraySum(discOut)`,
    `], sourceField:\`dt.entity.network:interface\`, lookupField:\`dt.entity.network:interface\`, prefix:"e."`,
    `| fields \`dt.entity.network:interface\`, if.interfaceName, d.deviceName, ifStatus, inLoad, outLoad, inTrafficBps, outTrafficBps, e.errorsTotal, e.discardsTotal`,
