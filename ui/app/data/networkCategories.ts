  ...
  /* ── 2. Reachability ───────────────────────────── */
  {
    ...
    kpi: {
      ...
      dqlQuery: [
        ...
        `| fieldsAdd reachPct = coalesce(record(avail.avgAvail)[takeMax(avail.avgAvail)], 100.0)`,
        `| filter reachPct < 100`,
  ...
  /* ── 3. Saturation ─────────────────────────────── */
  {
    ...
    kpi: {
      ...
      dqlQuery: [
        ...
        `| lookup [`,
        `  timeseries ifInBytes=sum(com.dynatrace.extension.network_device.if.bytes_in.count), by:{\`dt.entity.network:interface\`}`,
        `  | fieldsAdd ifInBytesMax = arrayMax(ifInBytes)`,
        `], sourceField:id, lookupField:\`dt.entity.network:interface\`, prefix:"in."`,
        `| lookup [`,
        `  timeseries ifOutBytes=sum(com.dynatrace.extension.network_device.if.bytes_out.count), by:{\`dt.entity.network:interface\`}`,
        `  | fieldsAdd ifOutBytesMax = arrayMax(ifOutBytes)`,
        `], sourceField:id, lookupField:\`dt.entity.network:interface\`, prefix:"out."`,
        `| fieldsAdd inLoad = (coalesce(in.ifInBytesMax, 0) * 8 / 300) / if_high_speed * 100`,
        `| fieldsAdd outLoad = (coalesce(out.ifOutBytesMax, 0) * 8 / 300) / if_high_speed * 100`,
        `| filter inLoad > 95 or outLoad > 95`,
      ...
      weekAgoDqlQuery: [
        ...
        `| fieldsAdd deviceName = d.deviceName`,
        `| lookup [`,
        `  timeseries ifInBytes=sum(com.dynatrace.extension.network_device.if.bytes_in.count), from: -7d-1d, to: -7d, by:{\`dt.entity.network:interface\`}`,
        `  | fieldsAdd ifInBytesMax = arrayMax(ifInBytes)`,
        `], sourceField:id, lookupField:\`dt.entity.network:interface\`, prefix:"in."`,
        `| lookup [`,
        `  timeseries ifOutBytes=sum(com.dynatrace.extension.network_device.if.bytes_out.count), from: -7d-1d, to: -7d, by:{\`dt.entity.network:interface\`}`,
        `  | fieldsAdd ifOutBytesMax = arrayMax(ifOutBytes)`,
        `], sourceField:id, lookupField:\`dt.entity.network:interface\`, prefix:"out."`,
        `| fieldsAdd inLoad = (coalesce(in.ifInBytesMax, 0) * 8 / 300) / if_high_speed * 100`,
        `| fieldsAdd outLoad = (coalesce(out.ifOutBytesMax, 0) * 8 / 300) / if_high_speed * 100`,
    ...
    secondaryKpi: {
      ...
      dqlQuery: [
        ...
        `| fieldsAdd outLoad = (coalesce(out.ifOutBytesMax, 0) * 8 / 300) / if_high_speed * 100`,
        `| filter inLoad > 95 or outLoad > 95`,
        `| summarize saturatedDevices = countDistinct(device)`,
      ].join('\n'),
      weekAgoDqlQuery: [
        `fetch \`dt.entity.network:interface\``,
        `| fieldsAdd device = \`belongs_to[\`dt.entity.network:device\`]\``,
  ...
  /* ── 5. Traffic ────────────────────────────────── */
  {
    ...
    secondaryKpi: {
      ...
      dqlQuery: [
        ...
        `  | fieldsAdd currentStatus = record(status)[takeMax(status)]`,
...
export const NETWORK_QUERIES = {
  ...
  deviceInventory: [
    ...
    `      record(memUsage)[takeMax(memUsage)],`,
    `      record(memUsed)[takeMax(memUsed)] / record(memTotal)[takeMax(memTotal)] * 100`,
    `    )`,
  ...
  interfaceHealth: [
    ...
    `| fieldsAdd inLoad = (coalesce(in.ifInBytesMax, 0) * 8 / 300) / if_high_speed * 100`,
    `| fieldsAdd outLoad = (coalesce(out.ifOutBytesMax, 0) * 8 / 300) / if_high_speed * 100`,
    `| fieldsAdd inTrafficBps = coalesce(in.ifInBytesMax, 0) * 8 / 300`,
    `| fieldsAdd outTrafficBps = coalesce(out.ifOutBytesMax, 0) * 8 / 300`,
    `| lookup [`,
    `  timeseries status=avg(com.dynatrace.extension.network_device.if.status), by:{\`dt.entity.network:interface\`}`,
    `  | fieldsAdd currentStatus = record(status)[takeMax(status)]`,
    `], sourceField:id, lookupField:\`dt.entity.network:interface\`, prefix:"s."`,
    `| fieldsAdd ifStatus = if(toDouble(s.currentStatus) >= 1.0, "up", "down")`,
    `| lookup [`,
    `  timeseries {`,
    `    errIn=sum(com.dynatrace.extension.network_device.if.in.errors.count),`,
    `    errOut=sum(com.dynatrace.extension.network_device.if.out.errors.count),`,
    ...
    `], sourceField:id, lookupField:\`dt.entity.network:interface\`, prefix:"e."`,
    `| fields id, entity.name, deviceName, ifStatus, inLoad, outLoad, inTrafficBps, outTrafficBps, e.errorsTotal, e.discardsTotal`,
    `| sort inLoad desc`,
    `| limit 100`,
  ].join('\n'),
  ...
  topologyEdges: [
    ...
    `    ifOutBytes = sum(com.dynatrace.extension.network_device.if.bytes_out.count)`,
    `  }, by:{\`dt.entity.network:interface\`}`,
    `  | fieldsAdd bitsPerSec = (arrayMax(ifInBytes) + arrayMax(ifOutBytes)) * 8 / 300`,
    `], sourceField:id, lookupField:\`dt.entity.network:interface\`, prefix:"bw."`,
    `| fieldsAdd ifSpeed = if_high_speed`,
